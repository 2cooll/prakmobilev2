...
class ExperimentController extends GetxController {
  ...
  Future<void> _httpRequest({bool includeDecode = false}) async {
    final res = await httpClient.get(Uri.parse(url), headers: {
      'Connection': 'keep-alive',
    });
  ...
  Future<void> runExperiment() async {
    ...
    for (int i = 0; i < runs; i++) {
      ...
      if (runOrder == 'HTTP-DIO') {
        final tHttp = await _measureNetworkFuture(() => _httpRequest(includeDecode: false));
        httpTimes.add(tHttp);
        log.value += 'HTTP Run ${i + 1}: ${tHttp}ms\n';

        final tDio = await _measureNetworkFuture(() => _dioRequest(includeDecode: false));
        dioTimes.add(tDio);
